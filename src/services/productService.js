...
const productService = {
    ...
    // It's a cleaner way to define the method compared to creating a separate constant.
        ...
        try {
            ...
            // Axios automatically parses JSON, so `response.data` contains the parsed data.(axios automatically convert the data into json to javascriptObject)
            return response.data.products;//.products means only product key can fetch []
    ...
    getProductById: async (productId) => {
        try {
            ...
            const response = await axios.get(`${API_URL}/${productId}`);
             // Axios automatically parses JSON, so `response.data` contains the parsed data.(axios automatically convert the data into json to javascriptObject)
            return response.data.product;//.products means only product key can fetch []
        } catch (error) {
    ...
    createProduct: async (productData, token) => {
        try {
            ...
            return response.data.product; //axios automatically converts the json formate into javascript object
        } catch (error) {
